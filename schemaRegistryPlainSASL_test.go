...
func createSchemaRegistryPlainSASLTestObject(t *testing.T, subject string, id int) *TestObject {
...
func TestSchemaRegistryPlainSASLClient_Retries(t *testing.T) {
	count := 0
...
func TestSchemaRegistryPlainSASLClient_Error(t *testing.T) {
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, `{"error_code": 500, "message": "Error in the backend datastore"}`, 500)
	}))
